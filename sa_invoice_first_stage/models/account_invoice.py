...
class Invoice(models.Model):
    ...
    qr_code_image = fields.Binary("QRCode Image", compute='_generate_qr_code',store=True)
    company_vat = fields.Char(string='Company / Vendor Vat',related="company_id.vat",store=True)

    @api.depends('company_vat','company_id','write_date','amount_total','amount_tax')
    def _generate_qr_code(self):
        for record in self:
            qr_info = ''
            required_fields = ['company_id', 'company_vat', 'write_date', 'amount_total', 'amount_tax']
            required_fields_attributes = self.env['ir.model.fields'].search([('model_id.model','=','account.invoice'), ('name','in',required_fields)])
            data = {}
            for field_info in required_fields_attributes:
                if field_info.name == "company_id":
                    tag = 1
                    value = record[field_info.name].display_name
                    if value:
            ...
            record.qr_code_image = base64.b64encode(data.getvalue()).decode()
